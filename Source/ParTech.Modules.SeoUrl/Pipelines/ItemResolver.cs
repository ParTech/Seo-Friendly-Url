namespace ParTech.Modules.SeoUrl.Pipelines
{
    ...
    public class ItemResolver : HttpRequestProcessor
    {
        ...
        private void ForceFriendlyUrl(HttpRequestArgs args)
        {
            ...
            string requestPath = args.Context.Request.Url.AbsolutePath;
            string rewriteModuleOriginalPath = Convert.ToString(Context.Items["Sitecore.Web.RewriteModule.OriginalUrl"]);

            // The language prefix may have been stripped by Sitecore before the request reached this ItemResolver.
            // In that case the requested path will never match the friendly path if language embedding is enabled.
            // For that reason, we use the original request URL that is tracked by the RewriteModule.
            string requestedPath = !string.IsNullOrEmpty(rewriteModuleOriginalPath)
                ? rewriteModuleOriginalPath
                : requestPath;

            // Get the URL path that is generated by the SEO-friendly LinkProvider.
